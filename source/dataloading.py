...
from .dataloaders.regression import LinearSystem, RegressionDataLoader
...
def create_linear_system_identification_dataset(
    cache_dir: Union[str, Path] = DEFAULT_CACHE_DIR_ROOT,
    seed: int = 42,
    batch_size: int = 128,
    **kwargs,
) -> ReturnType:
    print("[*] Generating linear system identification task")

    D_INPUT = kwargs["d_input"]
    D_OUTPUT = kwargs["d_output"]
    SEQ_LENGTH = kwargs["T"]
    TRAIN_SIZE = kwargs["n_batches_per_epoch"]
    VAL_SIZE = kwargs["n_batches_per_epoch"]

    dataset = LinearSystem(**kwargs)

    trn_loader = RegressionDataLoader(dataset, seed + 1, batch_size, TRAIN_SIZE)
    tst_loader = RegressionDataLoader(dataset, seed + 4315623, batch_size, VAL_SIZE)
    aux_loaders = {}
    return (
        trn_loader,
        None,
...
Datasets = {
    ...
    "linear-system-identification": create_linear_system_identification_dataset,
}
